    ...
    return (
        <div className="relative flex flex-col gap-8 max-w-viewport w-full mx-auto px-5 py-16 md:py-24 lg:py-32">
                    ...
                    <div className='flex'>
                        {testimonials.map((testimonial, index) => (
                            <div key={index} className="flex-[0_0_100%] min-w-0 pl-4 pr-4">
                                <AnimatedCard 
                                    className="w-full max-w-[calc(100vw-2.5rem)] sm:max-w-[500px] md:max-w-[600px] lg:max-w-[650px] mx-auto hover:bg-background-secondary" 
                                    disableScale 
                                    disableTextColor
                                >
            ...
            <div className="flex flex-row gap-3 md:gap-4 items-center justify-center">
                ...
                <div className="px-3 md:px-5 flex flex-row items-center gap-2">
                    {Array.from({ length: itemCount }).map((_, index) => (
                        <button 
                            key={index} 
                            className={cn(
                                "h-2 rounded-full transition-all duration-400",
                                index === currentIndex ? "bg-white w-5" : "bg-white/50 w-2"
                            )} 
                            onClick={() => {
