...
interface DitherEnterpriseStatueProps {
    className?: string;
...
export function DitherEnterpriseStatue({
    className = "",
    externalMousePos,
    isHovering = false,
    ...
    useEffect(() => {
        fetch(pointsUrl)
            ...
            .catch((err) => console.error("Failed to load enterprise statue points:", err));
    }, [pointsUrl]);
    ...
    const render = useCallback(() => {
        ...
        const dotSize = 1.6;
        const scale = Math.min(width, height) * 0.425;
