...
export function GitHubEmbedBlock({
  ...
  if (embedType === 'file' && parsed.path) {
    const rawUrl = `https://raw.githubusercontent.com/${parsed.owner}/${parsed.repo}/main/${parsed.path}`
    return (
      <div className={['not-prose', className].filter(Boolean).join(' ')}>
        <div className="bg-background-secondary border border-white/10 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-sm text-white">{parsed.path}</span>
            </div>
            <a
              href={url}
              target="_blank"
  ...
  if (loading) {
    return (
      <div className={['not-prose', className].filter(Boolean).join(' ')}>
        <div className="bg-background-secondary border border-white/10 rounded-xl p-6 animate-pulse">
  ...
  return (
    <div className={['not-prose', className].filter(Boolean).join(' ')}>
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="block bg-background-secondary border border-white/10 rounded-xl p-6 hover:border-accent/30 transition-colors group"
      >
