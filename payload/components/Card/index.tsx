...
export const Card: React.FC<{
  ...
  return (
    <article
      className={cn(
        'border border-foreground/10 rounded-lg overflow-hidden bg-background-secondary hover:cursor-pointer',
        className,
      )}
      ref={cardRef}
    >
      <div className="relative w-full ">
        ...
        {metaImage && typeof metaImage !== 'string' && <Media resource={metaImage} size="33vw" className='' />}
      </div>
      <div className="p-4">
        ...
        {description && (
          <p className="mt-2 text-foreground/70">{sanitizedDescription}</p>
        )}
      </div>
    </article>
  )
